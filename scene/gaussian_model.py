...
class GaussianModel:
    ...
    def __init__(self, args, rvq = True):
        ...
        self.vq_scale = ResidualVQ(dim = 3, codebook_size = args.rvq_size, num_quantizers = args.rvq_num, commitment_weight = 0., kmeans_init = True, kmeans_iters = 1, ema_update = False, learnable_codebook=True, in_place_codebook_optimizer=lambda *args, **kwargs: torch.optim.Adam(*args, **kwargs, lr=0.0001)).cuda()
        self.vq_rot = ResidualVQ(dim = 4, codebook_size = args.rvq_size, num_quantizers = args.rvq_num, commitment_weight = 0., kmeans_init = True, kmeans_iters = 1, ema_update = False, learnable_codebook=True, in_place_codebook_optimizer=lambda *args, **kwargs: torch.optim.Adam(*args, **kwargs, lr=0.0001)).cuda()
        self.rvq_bit = math.log2(args.rvq_size)
        self.rvq_num = args.rvq_num
        self.recolor = tcnn.Encoding(
                 ...
                 encoding_config={
                    ...
                    "log2_hashmap_size": args.max_hashmap,
                    "base_resolution": 16,
...
# , new_features_dc, new_features_rest,
    ...
    def mask_prune(self, iter):
        prune_mask = (torch.sigmoid(self._mask) <= 0.001).squeeze()
        self.prune_points(prune_mask, iter)
        torch.cuda.empty_cache()
